...
const getSalesAdd = (req, res) => {
  res.render("salesAdd");
};

const getSalesUpdate = (req, res) => {
  const sql = `SELECT * FROM sales NATURAL JOIN products WHERE id_sale = ${req.params.id_sale}`;
  connection.query(sql, (err, result) => {
    if (err) {
      console.log("Error al consultar: " + err);
    } else {
      res.render("salesUpdate", { data: result });
    }
  });
};
...
const getSalesDelete = (req, res) => {
  ...
  connection.query(sql, (err, result) => {
    if (err) {
      ...
      res.render("salesDelete", { data: result });
    }
  });
};
...
const getSeparado = (req, res) => {
  const sql = `SELECT * FROM sales NATURAL JOIN products WHERE payment_method ='separado'`;

  connection.query(sql, (err, result) => {
    if (err) {
      console.log("error al consultar separado: " + err);
    } else {
      // res.send('en pagina')
      // res.send(result)
      res.render("salesSeparado", {data: result});
    }
  });
};
